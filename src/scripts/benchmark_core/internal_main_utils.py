...
def _interval_membership_stats(intervals, y_value):
    merged = _merge_interval_list(intervals)
    if not merged:
        return False, np.nan, 0.0
    total_width = float(sum(high - low for low, high in merged))
    for low, high in merged:
        if low <= y_value <= high:
            return True, float(min(y_value - low, high - y_value)), total_width
    distance = min(min(abs(y_value - low), abs(y_value - high)) for low, high in merged)
    return False, float(distance), total_width
...
class Evaluator:
    ...
    def evaluate(uc_service, datasets, alphas, eval_config, mix_mem_data=None, evaluation_subset=None):
        ...
        for run_no, ((dataset_no, dataset), alpha) in enumerate(itertools.product(enumerate(datasets), alphas)):
            ...
            del prediction_log, prediction_log_artifacts, eps, calib_artifact, metrics_per_alpha, extra_dfs
            gc.collect()
    ...
    def evaluate_sota_on_validation(uc_service, datasets, alphas, no_calib=False, prefix="val"):
        ...
        overall_metrics_per_alpha = defaultdict(lambda: defaultdict(list))
        _discard(f"{prefix}/winkler_score_norm", summary='min', goal='minimize')
        for no_dataset, dataset in enumerate(datasets):
            ...
            for no_alpha, alpha in enumerate(alphas):
                ...
                del prediction_log, prediction_log_artifacts, eps, metrics_per_alpha
                gc.collect()
                if torch.cuda.is_available():
                    torch.cuda.empty_cache()
        to_log = {}
    ...
    def _setup_metrics(dataset):
        ...
        for prefix in [prefix_overall, prefix_per_ts]:
            _discard(f"{prefix}mean_coverage", step_metric='alpha', summary='none')
            prefixes[prefix].append("mean_coverage")
            _discard(f"{prefix}mean_coverage_eps", step_metric='alpha', summary='mean', goal='minimize')
            prefixes[prefix].append('mean_coverage_eps')
            _discard(f"{prefix}mean_pi_width", step_metric='alpha', summary='mean', goal='minimize')
            prefixes[prefix].append('mean_pi_width')
            _discard(f"{prefix}mean_pi_sd", step_metric='alpha', summary='none')
            prefixes[prefix].append('mean_pi_sd')
            _discard(f"{prefix}winkler_score", step_metric='alpha', summary='mean', goal='minimize')
            prefixes[prefix].append("winkler_score")
            _discard(f"{prefix}winkler_score_norm", step_metric='alpha', summary='mean', goal='minimize')
            prefixes[prefix].append("winkler_score_norm")

        # Per TS metrics
        for prefix in [prefix_per_ts]:
            _discard(f"{prefix}median_pi_with", step_metric='alpha', summary='mean', goal='minimize')
            #prefixes[prefix].append("median_pi_with")
            _discard(f"{prefix}mean_dist_to_bound_in", step_metric='alpha', summary='none')
            #prefixes[prefix].append("mean_dist_to_bound_in")
            _discard(f"{prefix}median_dist_to_bound_in", step_metric='alpha', summary='none')
            #prefixes[prefix].append("median_dist_to_bound_in")
            _discard(f"{prefix}mean_dist_to_bound_out", step_metric='alpha', summary='none')
            #prefixes[prefix].append("mean_dist_to_bound_out")
            _discard(f"{prefix}median_dist_to_bound_out", step_metric='alpha', summary='none')
            #prefixes[prefix].append("median_dist_to_bound_out")
            _discard(f"{prefix}mean_eps", step_metric='alpha', summary='none')
    ...
    def log_rolling_metrics(rolling_dfs, eval_config):
        if eval_config['rolling_per_ts']:
            ...
            for key, df_list in rolling_dfs.items():
                tables[f"{key}rolling_metrics"] = _table(dataframe=pd.concat(df_list))
            _discard(tables)
        if eval_config['rolling_overall']:
            for key, df_list in rolling_dfs.items():
                for df in df_list:
                    df['ts_key'] = key.split("_", 1)[1]
            _discard({f"RollingEval/rolling_metrics": _table(dataframe=pd.concat(
        ...
        if eval_config['rolling_as_list']:
            ...
            for key, df_list in rolling_dfs.items():
                for df in df_list:
                    df['ts_key'] = key.split("_", 1)[1]
                list_of_tables.append(_table(dataframe=pd.concat(df_list)))
            _discard({"RollingEval/rolling_metrics_list": list_of_tables})
    ...
    def _log_step(prediction: PIModelPrediction, pred_data: PIPredictionStepData, Y_step, rescale_param):
        ...
        if getattr(prediction, "pred_set", None) is not None:
            pred_set_norm = [(float(low), float(high)) for low, high in prediction.pred_set]
            pred_set = [(rescale(torch.tensor(low)).item(), rescale(torch.tensor(high)).item()) for low, high in pred_set_norm]
            step_log_dict['pred_set'] = pred_set
            step_log_dict['pred_set_norm'] = pred_set_norm
    ...
    def _log_eval_finished(uc_service: UncertaintyService, prediction_log: List, prediction_artifacts: List,
        ...
        if mask_full is not None and not result_df.empty:
            steps = result_df["step_overall"].astype(int).to_numpy()
            mask_values = mask_full.reshape(-1).detach().cpu().numpy()
            valid_steps = steps < mask_values.shape[0]
            valid_mask = np.zeros(result_df.shape[0], dtype=bool)
            valid_mask[valid_steps] = mask_values[steps[valid_steps]].astype(bool)
            result_df = result_df.loc[valid_mask].reset_index(drop=True)
        if 'pred_set' in result_df.columns:
            real_stats = result_df.apply(
                lambda row: _interval_membership_stats(row['pred_set'], row['y_real']),
                axis=1,
                result_type='expand',
            )
            norm_stats = result_df.apply(
                lambda row: _interval_membership_stats(row['pred_set_norm'], row['y_real_norm']),
                axis=1,
                result_type='expand',
            )
            result_df['y_in_pi'] = real_stats[0].astype(bool)
            result_df['y_to_pi_bound_dist'] = real_stats[1]
            result_df['pi_width'] = real_stats[2]
            result_df['y_to_pi_bound_dist_norm'] = norm_stats[1]
            result_df['pi_width_norm'] = norm_stats[2]
        else:
        ...
        export_dir = os.environ.get("POINTWISE_EXPORT_DIR")
        if export_dir:
            export_path = Path(export_dir)
            export_path.mkdir(parents=True, exist_ok=True)
            safe_ts_id = str(dataset.ts_id).replace("/", "_").replace(" ", "_")
            run_name = Path().cwd().name
            pointwise_df = result_df.copy()
            pointwise_df.insert(0, "alpha", alpha)
            pointwise_df.insert(0, "ts_id", dataset.ts_id)
            pointwise_df.insert(0, "run_dir", run_name)
            pointwise_df.to_csv(export_path / f"{run_name}__{safe_ts_id}__alpha_{alpha:g}.csv", index=False)
        if log_table:
            local_log[f"{ts_log_prefix}Table"] = result_df
        ...
        local_log.update(eval_with_prefix)
        ...
        if log_pred_vega:
            ...
            local_log[f"{ts_log_prefix_plots}_A{alpha}_Prediction_Plot"] = \
                _plot_table(vega_spec_name="aauer/tspredictionwithinterval",
                                 data_table=_table(dataframe=data_table),
        ...
        if log_att_vega and 'uc_attention' in prediction_artifacts and is_first_dataset:  # Only For first dataset
            ...
            att_table = _table(dataframe=pd.DataFrame({'pred_step': col_pred_steps, 'source_step': col_source_steps,
                                   "attention": col_attention, 'y_hat': col_y_hats, "y": col_y}))
            local_log[f"{ts_log_prefix_plots}_A{alpha}_Attention_Plot"] = \
                _plot_table(vega_spec_name="aauer/tsattention",
        ...
        if log_pred_matplotlib:
            ...
            local_log[f"{ts_log_prefix_plots}Prediction_Plot (Img)"] = _image(fig, caption=f"Alpha: {alpha}")
            ax.set_title(f"Prediction Plot (Alpha: {alpha})")
            if print_pyplot_figs:
                local_log[f"{ts_log_prefix_plots}Prediction_Plot"] = fig
        ...
        if log_eps_matplotlib and eps is not None:
            ...
            ax.set_xlim(dataset.first_prediction_step, dataset.no_of_steps)
            local_log[f"{ts_log_prefix_plots}Epsilon_Plot (Img)"] = _image(fig, caption=f"Alpha: {alpha}")
            ax.set_title(f"Epsilon Plot (Alpha: {alpha})")
            if print_pyplot_figs:
                local_log[f"{ts_log_prefix_plots}Epsilon:Plot"] = fig
        ...
        if not isinstance(dataset, BoostrapEnsembleTsDataset) and print_full_ts_matplotlib:
            ...
            local_log[f"{ts_log_prefix_plots}Full_TS_Plot (Img)"] = _image(fig)
            ax.set_title(f"Full TS Plot (Alpha: {alpha})")
            if print_pyplot_figs:
                local_log[f"{ts_log_prefix_plots}Full_TS_Plot"] = fig
        ...
        if plot_ensemble_matplotlib and fc_service.has_ensemble:
            ...
            local_log[f"{ts_log_prefix_plots}Ensemble_Plot"] = fig
        ...
        if log_att_matplotlib and 'uc_attention' in prediction_artifacts:
            ...
            for head, histogram in histograms_gathered.items():
                ...
                local_log[f'{ts_log_prefix_plots}Attention Head {head}'] = fig
