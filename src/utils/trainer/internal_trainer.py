...
class ModelInternalTrainer(BaseTrainer):
    ...
    def _train_epoch(self, epoch: int) -> None:
        ...
        for batch_idx, batch_data in enumerate(pbar):
            ...
            LOGGER.debug({f"{_TRAIN_STEP_PREFIX}{key}": val for key, val in step_log_dict.items()})
        ...
        LOGGER.debug(dict(**{f"{_TRAIN_EP_PREFIX}{key}": val for key, val in log_dict.items()}, epoch=epoch))
    ...
    def _val_epoch(self, epoch: int, trained_model: nn.Module) -> float:
        ...
        LOGGER.debug(dict(**{f"{_VAL_EP_PREFIX}{key}": val for key, val in log_dict.items()}, epoch=epoch))
        #LOGGER.info(f'Val epoch \n{pd.Series(convert_dict_to_python_types(log_dict), dtype=float)}')

        reset_old_score = False
        use_threshold_pi = (
            hasattr(self._trainer_config, 'model_selection')
            and self._trainer_config.model_selection == 'threshold-pi'
            and 'MissCoverage' in metric_vals
            and 'PIWidth' in metric_vals
        )
        if use_threshold_pi:
    ...
    def _final_hook(self, final_results: Dict[str, Any], *args, **kwargs):
        LOGGER.info(f"Final training results: {final_results}")
